...
parser.add_argument(
    "-c", "--config", type=str, default="../experiments/Chicken/config_exemplar.yaml", help="Path of config"
)
parser.add_argument("-e", "--evaluate", action="store_false")
...
def main():
    ...
    train_loader, val_loader, test_loader = build_dataloader(
        config.dataset, distributed=True if local_rank != -1 else False
    )
